...
jest.mock('@/shared/database/prisma', () => ({
  __esModule: true,
  prisma: { assetAssignment: mockAssetAssignment },
  default: { assetAssignment: mockAssetAssignment },
}));
...
describe('AssetRepository employee self-service', () => {
  ...
  it('scopes active assignments to the authenticated company and employee', async () => {
    mockAssetAssignment.findMany.mockResolvedValue([{ id: 'assignment-1' }]);
    mockAssetAssignment.count.mockResolvedValue(1);
    ...
    expect(mockAssetAssignment.findMany).toHaveBeenCalledWith(expect.objectContaining({
      where: {
        companyId: 'company-1',
    ...
    expect(mockAssetAssignment.count).toHaveBeenCalledWith({
      where: {
        companyId: 'company-1',
        employeeId: 'employee-1',
        asset: { deletedAt: null },
  ...
  it('can return history without exposing financial asset fields', async () => {
    mockAssetAssignment.findMany.mockResolvedValue([]);
    mockAssetAssignment.count.mockResolvedValue(0);
    ...
    const call = mockAssetAssignment.findMany.mock.calls[0][0];
