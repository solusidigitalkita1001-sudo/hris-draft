...
export class WorkCalendarController {
  ...
  async createShiftFormula(req: AuthenticatedRequest, res: Response, next: NextFunction) {
    try {
      const code = await generateSystemCode({
        prefix: 'SHIFT',
        label: req.body.name,
        exists: async (candidate) => Boolean(await workCalendarRepository.findShiftFormulaByCode(req.body.companyId, candidate)),
      });
      const data = await workCalendarRepository.createShiftFormula({ ...req.body, code, createdBy: req.user!.id });
      res.status(201).json(Result.created(data));
    } catch (error) { next(error); }
  }
  ...
  async updateShiftFormula(req: Request, res: Response, next: NextFunction) {
    try {
      const current = await workCalendarRepository.findShiftFormulaById(req.params.sid as string);
      if (!current) return res.status(404).json(Result.error('Shift formula not found'));
      if (req.body.code && req.body.code !== current.code) {
        throw new ValidationError('Shift formula code is generated by system and cannot be changed');
      }
      if (req.body.code === current.code) {
